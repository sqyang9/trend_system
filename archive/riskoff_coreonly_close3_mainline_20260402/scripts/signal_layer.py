...
try:
    ...
    from v121_coreonly_riskoff_sellside_ema_audit import build_indicators, build_target  # noqa: E402
    from v123_formal_launch_and_layer2_weight_audit import SCENARIOS, base_bundle, simulate_weight_combo  # noqa: E402
finally:
...
def build_signal_snapshot() -> Dict:
    ...
    riskoff_params = riskoff_current_research_optimal(**SCENARIOS[0]["riskoff_overrides"])
    riskoff_ind = build_indicators(df_4h, riskoff_params, 250)
    ...
    core_exp = (bundle["base_core_exposure"] * ADOPTED_POSTURE["core_weight"]).reindex(idx).astype(float)
    s1_exp = (bundle["base_s1_weight"] * ADOPTED_POSTURE["sleeve1_weight"]).reindex(idx).astype(float)
    s2_exp = (bundle["base_s2_weight"] * ADOPTED_POSTURE["sleeve2_weight"]).reindex(idx).astype(float)
    ...
    recent_4h["s1_weight"] = (
        (bundle["base_s1_weight"] * ADOPTED_POSTURE["sleeve1_weight"]).reindex(recent_4h.index).fillna(0.0).astype(float)
    )
    recent_4h["s2_weight"] = (
        (bundle["base_s2_weight"] * ADOPTED_POSTURE["sleeve2_weight"]).reindex(recent_4h.index).fillna(0.0).astype(float)
    )
    ...
    payload = {
        ...
        "recent_ohlc": recent_4h[["open", "high", "low", "close", "ema250", "core_target", "s1_weight", "s2_weight"]],
