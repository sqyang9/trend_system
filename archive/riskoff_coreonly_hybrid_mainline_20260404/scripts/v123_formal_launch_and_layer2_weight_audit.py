...
ADOPTED_SPEC = {
    "name": "EMA250_CLOSE3_HC23_STRICT_BREAKOUT4",
    "label": "EMA250 close3 / HC23 strict breakout4",
...
def base_bundle(df_5m, df_4h, scenario: Dict) -> Dict:
...
def simulate_weight_combo(bundle: Dict, spec: Dict) -> Dict:
    ...
    combo_equity = (10000.0 + (core_pnl + s1_pnl + s2_pnl).cumsum()).astype(float)
    combo_exposure = (
        bundle["base_core_exposure"] * spec["core_weight"]
        + bundle["base_s1_weight"] * spec["sleeve1_weight"]
        + bundle["base_s2_weight"] * spec["sleeve2_weight"]
    ).astype(float)
