...
- adopted core-only Risk-Off overlay:
  ...
  - canonical adopted overlay version:
    - `Formal + EMA250_CLOSE3_HC23_STRICT_BREAKOUT4`
  - overlay-level adopted readout:
    - `Return 2459.02%`
    - `Calmar 2.352`
    - `MaxDD -29.06%`
  - standing retained non-primary reference:
...
- isolated volatility proxy system-defect research:
  ...
  - current readout:
    ...
    - best combined independent probe:
      ...
      - `MaxDD -24.17%`
  - current status:
    - promising independent challenger family
    - not promoted
    - keep as active reserve research until a later explicit promotion audit
...
- Use the adopted core-only `EMA250 + stable close3 + HC23 strict breakout_4 + Weekly RSI30 override` line as the canonical Risk-Off reference.
