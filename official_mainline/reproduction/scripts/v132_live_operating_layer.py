...
ADOPTED_POSTURE = {
    ...
    **ADOPTED_S1_GATE_CONTRACT,
    **ADOPTED_ATRVT_CONTRACT["atrvt_s1_spec"],
...
def build_live_package() -> Dict:
    ...
    bundle = base_bundle(df_5m, df_4h, SCENARIOS[0], atrvt_spec=ADOPTED_POSTURE)
    ...
    s1_gate_trace = bundle["s1_gate_trace"].reindex(idx)
    s1_gate_trace["s1_gate_enabled"] = s1_gate_trace["s1_gate_enabled"].fillna(bool(bundle["s1_gate_contract"]["s1_gate_enabled"]))
    s1_gate_trace["s1_gate_family"] = s1_gate_trace["s1_gate_family"].fillna(str(bundle["s1_gate_contract"]["s1_gate_family"]))
    s1_gate_trace["s1_gate_label"] = s1_gate_trace["s1_gate_label"].fillna(str(bundle["s1_gate_contract"]["s1_gate_label"]))
    ...
    latest_atr_scale_s1 = float(sim["atr_scale_s1"].reindex(idx).ffill().iloc[-1])
    latest_atr_scale_s2 = float(sim["atr_scale_s2"].reindex(idx).ffill().iloc[-1])
    ...
    latest = {
        ...
        "atr_scale_s1": latest_atr_scale_s1,
        "atr_scale_s2": latest_atr_scale_s2,
        "hv_pct_180d": latest_hv_pct,
        "atrvt_label": str(sim["atrvt_label"]),
        "atrvt_s1_label": str(sim["atrvt_s1_label"]),
        "atrvt_s2_label": str(sim["atrvt_s2_label"]),
        "atr_ref_days": int(sim["atrvt_s1_spec"]["atr_ref_days"]),
        "atr_ref_stat": str(sim["atrvt_s1_spec"]["atr_ref_stat"]),
        "atr_scale_min": float(sim["atrvt_s1_spec"]["atr_scale_min"]),
        "atr_scale_max": float(sim["atrvt_s1_spec"]["atr_scale_max"]),
        "atrvt_s1_ref_days": int(sim["atrvt_s1_spec"]["atr_ref_days"]),
        "atrvt_s1_ref_stat": str(sim["atrvt_s1_spec"]["atr_ref_stat"]),
        "atrvt_s1_scale_min": float(sim["atrvt_s1_spec"]["atr_scale_min"]),
        "atrvt_s1_scale_max": float(sim["atrvt_s1_spec"]["atr_scale_max"]),
        "atrvt_s2_ref_days": int(sim["atrvt_s2_spec"]["atr_ref_days"]),
        "atrvt_s2_ref_stat": str(sim["atrvt_s2_spec"]["atr_ref_stat"]),
        "atrvt_s2_scale_min": float(sim["atrvt_s2_spec"]["atr_scale_min"]),
        "atrvt_s2_scale_max": float(sim["atrvt_s2_spec"]["atr_scale_max"]),
        "s1_gate_enabled": bool(s1_gate_trace["s1_gate_enabled"].iloc[-1]),
        "s1_gate_family": str(s1_gate_trace["s1_gate_family"].iloc[-1]),
        "s1_gate_label": str(s1_gate_trace["s1_gate_label"].iloc[-1]),
        "s1_gate_candidate": bool(s1_gate_trace["s1_candidate"].iloc[-1]),
        "s1_gate_pass": bool(s1_gate_trace["s1_gate_pass"].iloc[-1]),
        "s1_gate_reason": str(s1_gate_trace["s1_gate_reason"].iloc[-1]),
        "s1_vp_hvn_escape_atr": (
            float(s1_gate_trace["s1_vp_hvn_escape_atr"].iloc[-1])
            if pd.notna(s1_gate_trace["s1_vp_hvn_escape_atr"].iloc[-1])
            else None
        ),
        "s1_vp_volume_ratio20": (
            float(s1_gate_trace["s1_vp_volume_ratio20"].iloc[-1])
            if pd.notna(s1_gate_trace["s1_vp_volume_ratio20"].iloc[-1])
            else None
        ),
    ...
    return {
        ...
        "s1_gate_trace": s1_gate_trace,
        "atr_scale": sim["atr_scale"].reindex(idx).astype(float),
        "atr_scale_s1": sim["atr_scale_s1"].reindex(idx).astype(float),
        "atr_scale_s2": sim["atr_scale_s2"].reindex(idx).astype(float),
        "hv_pct_180d": sim["hv_pct_180d"].reindex(idx).astype(float),
        "atrvt_label": str(sim["atrvt_label"]),
        "atrvt_contract": sim["atrvt_contract"],
    }
...
def memo_text(pkg: Dict) -> str:
    ...
    lines = [
        ...
        f"- S1 gate: `{latest['s1_gate_label']}` / latest reason `{latest['s1_gate_reason']}`",
        "",
        "## Operating Readout",
        "",
...
def playbook_text(pkg: Dict) -> str:
    ...
    lines = [
        ...
        f"- Sleeve scaling: `S1 + S2 ATR vol-targeting` via `{latest['atrvt_label']}`.",
        f"  - `S1={latest['atrvt_s1_label']}`",
        f"  - `S2={latest['atrvt_s2_label']}`",
        f"- Adopted `S1` gate contract: `{latest['s1_gate_label']}`.",
...
def build_html(pkg: Dict) -> str:
    ...
    fig.update_layout(
        ...
        title=(
            ...
            f"ATR={latest['atr_scale']:.2f}x | S1={latest['atr_scale_s1']:.2f}x S2={latest['atr_scale_s2']:.2f}x | ATRVT={latest['atrvt_s1_ref_days']}d/{latest['atrvt_s2_ref_days']}d | HV={latest['hv_pct_180d'] * 100.0:.1f} | Onboarding={pkg['preferred_onboarding']}</sup>"
        ),
    )
        ...
        json.dumps(
            {
                ...
                "s1_gate_contract": {
                    "label": pkg["latest"]["s1_gate_label"],
                    "enabled": pkg["latest"]["s1_gate_enabled"],
                    "family": pkg["latest"]["s1_gate_family"],
                    "candidate": pkg["latest"]["s1_gate_candidate"],
                    "pass": pkg["latest"]["s1_gate_pass"],
                    "reason": pkg["latest"]["s1_gate_reason"],
                    "hvn_escape_atr": pkg["latest"]["s1_vp_hvn_escape_atr"],
                    "volume_ratio20": pkg["latest"]["s1_vp_volume_ratio20"],
                },
            },
            ensure_ascii=False,
            indent=2,
