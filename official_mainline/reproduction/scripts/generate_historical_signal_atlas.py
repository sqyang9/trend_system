...
from v123_formal_launch_and_layer2_weight_audit import ADOPTED_SPEC, SCENARIOS, base_bundle, simulate_weight_combo
from live_operating_layer.v132_live_operating_layer import ADOPTED_POSTURE


ROOT = Path(__file__).resolve().parent
OUTDIR = ROOT / "output_vp_gate_mainline"
INDEX_CSV = OUTDIR / "atlas_index.csv"
SUMMARY_MD = OUTDIR / "atlas_summary.md"
TRADES_CSV = OUTDIR / "mainline_trade_ledger.csv"
...
def to_utc_ts(value) -> pd.Timestamp:
...
def build_trade_ledger(bundle: dict) -> pd.DataFrame:
    rows: list[dict] = []

    for seq, rec in enumerate(bundle["core_sim"]["transitions"], start=1):
        rows.append(
            {
                "sleeve": "core",
                ...
                "seq": seq,
                "trade_id": None,
                "signal_time": to_utc_ts(rec["signal_time"]),
                "execution_time": to_utc_ts(rec["execution_time"]),
                "entry_time": pd.NaT,
                "exit_time": pd.NaT,
                "trade_side": str(rec["trade_side"]),
                "target_weight": float(rec["target_weight"]),
                "exposure_weight": float(rec["target_weight"]),
                "entry_price": None,
                "exit_price": None,
                "fill_price": float(rec["fill_price"]),
                "trade_notional": float(rec["trade_notional"]),
                "exit_reason": None,
    ...
    s1_plan = bundle.get("s1_gate_plan", pd.DataFrame())
    if s1_plan is not None and not s1_plan.empty:
        s1_exec = s1_plan.loc[s1_plan["addon_weight"].astype(float) > 1e-9].copy()
        for _, rec in s1_exec.iterrows():
            ...
            rows.append(
                {
                    ...
                    "entry_time": to_utc_ts(rec["entry_time"]),
                    "exit_time": to_utc_ts(rec["exit_time"]),
                    "trade_side": "buy_then_sell",
                    "target_weight": float(rec["addon_weight"]),
                    "exposure_weight": float(rec["addon_weight"]),
                    "entry_price": entry_price,
                    "exit_price": exit_price,
                    "fill_price": None,
                    "trade_notional": None,
                    "exit_reason": str(rec["exit_reason"]),
                    "gross_return_pct": float((exit_price / entry_price - 1.0) * 100.0) if entry_price > 0 else None,
                    "gate_label": bundle["s1_gate_label"],
                    "gate_reason": "pass",
                }
            )

    s2_meta = bundle.get("s2_trade_meta", pd.DataFrame())
    if s2_meta is not None and not s2_meta.empty:
        tmp = s2_meta.copy()
        tmp["entry_time"] = pd.to_datetime(tmp["entry_time"], utc=True)
        tmp["exit_time"] = pd.to_datetime(tmp["exit_time"], utc=True)
        for trade_id, rec in tmp.reset_index(drop=True).iterrows():
            entry_price = float(rec["entry_price"])
            exit_price = float(rec["exit_price"])
            rows.append(
                {
    ...
    if ledger.empty:
        return ledger
    ledger["sort_time"] = ledger["execution_time"]
    ledger["sort_time"] = ledger["sort_time"].fillna(ledger["entry_time"])
    ledger["sort_time"] = ledger["sort_time"].fillna(ledger["signal_time"])
    ledger = ledger.sort_values(["sort_time", "sleeve", "trade_id"], kind="stable").reset_index(drop=True)
    return ledger.drop(columns=["sort_time"])


def build_full_package() -> dict:
    df_5m, df_4h = load_full_history()
    scenario = SCENARIOS[0]
    bundle = base_bundle(df_5m, df_4h, scenario, atrvt_spec=ADOPTED_POSTURE)
    sim = simulate_weight_combo(bundle, ADOPTED_POSTURE)
    indicators = bundle["core_indicators_4h"]
    core_trace = bundle["core_trace_4h"]
    s1_gate_trace = bundle["s1_gate_trace"]
    core_target_unit = bundle["core_target_4h"].astype(float)
    instability = bundle["core_instability_4h"]
    ...
    recent["ema50"] = indicators["ema50"].reindex(recent.index).astype(float)
    recent["core_target"] = (
        core_target_unit.reindex(recent.index).ffill().fillna(1.0).astype(float) * ADOPTED_POSTURE["core_weight"]
    )
    recent["core_gate"] = core_trace["active_gate"].reindex(recent.index).ffill().fillna("baseline_close3")
    recent["core_instability_state"] = core_trace["instability_state"].reindex(recent.index).ffill().fillna("stable")
    recent["s1_weight"] = sim["effective_s1_weight"].reindex(recent.index).fillna(0.0).astype(float)
    recent["s2_weight"] = sim["effective_s2_weight"].reindex(recent.index).fillna(0.0).astype(float)
    recent["s1_gate_candidate"] = s1_gate_trace["s1_candidate"].reindex(recent.index).fillna(False)
    recent["s1_gate_pass"] = s1_gate_trace["s1_gate_pass"].reindex(recent.index).fillna(False)
    recent["s1_gate_reason"] = s1_gate_trace["s1_gate_reason"].reindex(recent.index).fillna(
        "disabled" if not bool(bundle["s1_gate_contract"]["s1_gate_enabled"]) else "not_applicable"
    )
    ...
    s1_blocked = recent[(recent["s1_gate_candidate"]) & (~recent["s1_gate_pass"])].copy()
    s1_blocked["timestamp"] = s1_blocked.index

    return {
        "ohlc": recent[["open", "high", "low", "close", "ema250", "ema50", "core_target", "s1_weight", "s2_weight", "s1_gate_candidate", "s1_gate_pass", "s1_gate_reason"]],
        "equity": equity,
        "total_exposure": total_exposure,
        "trade_ledger": build_trade_ledger(bundle),
        ...
        "s1_blocked": s1_blocked[["timestamp", "s1_gate_reason"]].copy(),
        "core_flat_spans": contiguous_flat_spans(recent["core_target"]),
        "instability": recent["core_instability_state"].copy(),
        "atrvt_label": str(sim["atrvt_label"]),
        "s1_gate_label": str(bundle["s1_gate_label"]),
    }
...
def draw_labeled_events(ax, price_df: pd.DataFrame, events: pd.DataFrame, kind: str) -> None:
    ...
    spec = {
        ...
        "s1_blocked": {"marker": "D", "color": "#64748b", "text": "S1X", "va": "top"},
...
def render_window(
    ...
    s1_blocked = subset_events(package["s1_blocked"], start, end)
    flat_spans = [(a, b) for a, b in package["core_flat_spans"] if b >= start and a <= end]
    instability = package["instability"].loc[(package["instability"].index >= start) & (package["instability"].index <= end)].copy()
    ...
    unstable_mask = instability == "highly_unstable"
    unstable_start = None
    prev_ts = None
    for ts, is_unstable in unstable_mask.items():
        if is_unstable and unstable_start is None:
            unstable_start = ts
        if not is_unstable and unstable_start is not None:
            ax_price.axvspan(
                mdates.date2num(pd.Timestamp(unstable_start).to_pydatetime()),
                mdates.date2num(pd.Timestamp(prev_ts if prev_ts is not None else ts).to_pydatetime()),
                color="#fef3c7",
                alpha=0.22,
            )
            unstable_start = None
        prev_ts = ts
    if unstable_start is not None and prev_ts is not None:
        ax_price.axvspan(
            mdates.date2num(pd.Timestamp(unstable_start).to_pydatetime()),
            mdates.date2num(pd.Timestamp(prev_ts).to_pydatetime()),
            color="#fef3c7",
            alpha=0.22,
        )
    ...
    ax_price.plot(ohlc.index, ohlc["ema50"].astype(float).values, color="#7c3aed", linewidth=1.2, alpha=0.9, label="EMA50")
    ...
    draw_labeled_events(ax_price, ohlc, s1_blocked, "s1_blocked")
    ...
    meta_lines = [
        ...
        f"S1 blocked: {int(len(s1_blocked))}",
        f"S2 buy/sell: {int((s2_events['event'] == 'entry').sum())}/{int((s2_events['event'] == 'exit').sum())}",
        f"S1 gate contract: {package['s1_gate_label']}",
        "Legend: grey zone=core flat | RE=core/re-entry restore | FLAT=core flat | S1B/S1S | S1X=blocked S1 candidate | S2B/S2S",
    ]
    ...
    return {
        ...
        "s1_blocked_count": int(len(s1_blocked)),
...
def main() -> None:
    ...
    package["trade_ledger"].to_csv(TRADES_CSV, index=False)
    lines = [
        ...
        f"- trade ledger: `{TRADES_CSV.name}`",
