...
from v90_addon_grading_study import constant_weight_plan, prepare_base_run, simulate_sleeve
...
from s1_gate_background import DEFAULT_S1_GATE_CONTRACT, build_s1_gate_features, normalize_s1_gate_contract
from volatility_background import atrvt_contract_label, atrvt_label, build_volatility_background, normalize_atrvt_contract, normalize_atrvt_spec
...
ADOPTED_ATRVT_CONTRACT = normalize_atrvt_contract(
    {
        "atrvt_s1_ref_days": 90,
        "atrvt_s1_ref_stat": "median",
        "atrvt_s1_scale_min": 0.35,
        "atrvt_s1_scale_max": 1.50,
        "atrvt_s2_ref_days": 60,
        "atrvt_s2_ref_stat": "median",
        "atrvt_s2_scale_min": 0.35,
        "atrvt_s2_scale_max": 1.50,
    }
)
ADOPTED_S1_GATE_CONTRACT = normalize_s1_gate_contract(
    {
        "s1_gate_enabled": True,
        "s1_gate_family": "volume_profile_proxy",
        "s1_vp_lookback": 60,
        "s1_vp_escape_atr": 0.50,
        "s1_vp_volume_ratio": 1.20,
    }
)
...
def _build_s1_gate_bundle(df_5m, df_4h, scenario: Dict, gate_cfg: Dict) -> Dict:
    d4 = ensure_datetime(df_4h).set_index("timestamp").sort_index()
    trace = pd.DataFrame(index=d4.index)
    trace["s1_gate_enabled"] = bool(gate_cfg["s1_gate_enabled"])
    trace["s1_gate_family"] = str(gate_cfg["s1_gate_family"])
    trace["s1_gate_label"] = str(gate_cfg["s1_gate_label"])
    trace["s1_candidate"] = False
    trace["s1_gate_pass"] = False
    trace["s1_gate_reason"] = "disabled" if not gate_cfg["s1_gate_enabled"] else "not_applicable"
    trace["s1_vp_hvn_escape_atr"] = np.nan
    trace["s1_vp_volume_ratio20"] = np.nan
    if not gate_cfg["s1_gate_enabled"]:
        return {"trace": trace, "plan": pd.DataFrame(), "entries": pd.DataFrame()}
    ...
    feat = build_s1_gate_features(df_4h, gate_cfg).set_index("signal_bar")
    merged = entries.merge(feat.reset_index(), on="signal_bar", how="left")
    plan = constant_weight_plan(merged, 1.0)
    plan["addon_weight"] = np.where(merged["s1_gate_pass"].fillna(False), 1.0, 0.0)
    sleeve = simulate_sleeve(df_4h, plan, float(formal_params.commission_pct))

    cand_bars = pd.Index(merged["signal_bar"])
    trace.loc[cand_bars, "s1_candidate"] = True
    trace.loc[cand_bars, "s1_gate_pass"] = merged["s1_gate_pass"].fillna(False).to_numpy()
    trace.loc[cand_bars, "s1_gate_reason"] = merged["s1_gate_reason"].fillna("not_applicable").astype(str).to_numpy()
    trace.loc[cand_bars, "s1_vp_hvn_escape_atr"] = merged["s1_vp_hvn_escape_atr"].to_numpy()
    trace.loc[cand_bars, "s1_vp_volume_ratio20"] = merged["s1_vp_volume_ratio20"].to_numpy()
    return {
        "trace": trace,
        "sleeve_equity": sleeve["equity"].astype(float),
        "sleeve_weight": sleeve["weight"].astype(float),
        "plan": plan.copy(),
        "entries": merged.copy(),
    }
...
def base_bundle(
    ...
    df_4h,
    scenario: Dict,
    adopted_spec: Dict | None = None,
    atrvt_spec: Dict | None = None,
    s1_gate_contract: Dict | None = None,
) -> Dict:
    spec = ADOPTED_SPEC if adopted_spec is None else adopted_spec
    contract_cfg = ADOPTED_ATRVT_CONTRACT if atrvt_spec is None else normalize_atrvt_contract(atrvt_spec)
    gate_cfg = (
        ADOPTED_S1_GATE_CONTRACT
        if atrvt_spec is None and s1_gate_contract is None
        else normalize_s1_gate_contract(atrvt_spec if s1_gate_contract is None else s1_gate_contract)
    )
    formal_params = formal_current_research_optimal(**scenario["formal_overrides"])
    ...
    s1_background = build_volatility_background(
        df_4h,
        atr_ref_days=contract_cfg["atrvt_s1_spec"]["atr_ref_days"],
        atr_ref_stat=contract_cfg["atrvt_s1_spec"]["atr_ref_stat"],
        atr_scale_min=contract_cfg["atrvt_s1_spec"]["atr_scale_min"],
        atr_scale_max=contract_cfg["atrvt_s1_spec"]["atr_scale_max"],
    )
    if contract_cfg["atrvt_s1_label"] == contract_cfg["atrvt_s2_label"]:
        s2_background = s1_background
    else:
        s2_background = build_volatility_background(
            df_4h,
            atr_ref_days=contract_cfg["atrvt_s2_spec"]["atr_ref_days"],
            atr_ref_stat=contract_cfg["atrvt_s2_spec"]["atr_ref_stat"],
            atr_scale_min=contract_cfg["atrvt_s2_spec"]["atr_scale_min"],
            atr_scale_max=contract_cfg["atrvt_s2_spec"]["atr_scale_max"],
        )
    ...
    s1_gate_bundle = _build_s1_gate_bundle(df_5m, df_4h, scenario, gate_cfg)
    if gate_cfg["s1_gate_enabled"]:
        base_s1_equity = s1_gate_bundle["sleeve_equity"].reindex(idx).ffill().bfill().astype(float)
        base_s1_weight = s1_gate_bundle["sleeve_weight"].reindex(idx).fillna(0.0).astype(float)
        base_s1_pnl = (base_s1_equity.diff().fillna(0.0)).astype(float)
    s1_gate_trace = s1_gate_bundle["trace"].reindex(idx)
    s1_gate_trace["s1_gate_enabled"] = s1_gate_trace["s1_gate_enabled"].fillna(bool(gate_cfg["s1_gate_enabled"]))
    s1_gate_trace["s1_gate_family"] = s1_gate_trace["s1_gate_family"].fillna(str(gate_cfg["s1_gate_family"]))
    s1_gate_trace["s1_gate_label"] = s1_gate_trace["s1_gate_label"].fillna(str(gate_cfg["s1_gate_label"]))
    s1_gate_trace["s1_candidate"] = s1_gate_trace["s1_candidate"].fillna(False)
    s1_gate_trace["s1_gate_pass"] = s1_gate_trace["s1_gate_pass"].fillna(False)
    s1_gate_trace["s1_gate_reason"] = s1_gate_trace["s1_gate_reason"].fillna(
        "disabled" if not gate_cfg["s1_gate_enabled"] else "not_applicable"
    )
    ...
    return {
        ...
        "core_sim": core_sim,
        "s1_gate_plan": s1_gate_bundle.get("plan", pd.DataFrame()).copy(),
        "s1_gate_entries": s1_gate_bundle.get("entries", pd.DataFrame()).copy(),
        "s2_trade_meta": (
            formal_payload["candidate_result"].get("trade_meta").copy()
            if formal_payload["candidate_result"].get("trade_meta") is not None
            else pd.DataFrame()
        ),
        "s1_gate_contract": gate_cfg,
        "s1_gate_label": gate_cfg["s1_gate_label"],
        "s1_gate_trace": s1_gate_trace.copy(),
        "atrvt_contract": contract_cfg,
        "atrvt_contract_label": atrvt_contract_label(contract_cfg),
        "atrvt_s1_spec": contract_cfg["atrvt_s1_spec"],
        "atrvt_s2_spec": contract_cfg["atrvt_s2_spec"],
        "atrvt_s1_label": contract_cfg["atrvt_s1_label"],
        "atrvt_s2_label": contract_cfg["atrvt_s2_label"],
        "atrvt_spec": contract_cfg["atrvt_s1_spec"],
        "atrvt_label": atrvt_contract_label(contract_cfg),
        "atr_scale": s1_background["atr_scale"].reindex(idx).ffill().fillna(1.0).astype(float),
        "atr_scale_s1": s1_background["atr_scale"].reindex(idx).ffill().fillna(1.0).astype(float),
        "atr_scale_s2": s2_background["atr_scale"].reindex(idx).ffill().fillna(1.0).astype(float),
...
def simulate_weight_combo(bundle: Dict, spec: Dict) -> Dict:
    ...
    if use_s1_atr:
        s1_scale = bundle["atr_scale_s1"] * float(spec["sleeve1_weight"])
    ...
    if use_s2_atr:
        s2_scale = bundle["atr_scale_s2"] * float(spec["sleeve2_weight"])
    ...
    return {
        ...
        "atrvt_contract": bundle["atrvt_contract"],
        "atrvt_contract_label": bundle["atrvt_contract_label"],
        "atrvt_s1_spec": bundle["atrvt_s1_spec"],
        "atrvt_s2_spec": bundle["atrvt_s2_spec"],
        "atrvt_s1_label": bundle["atrvt_s1_label"],
        "atrvt_s2_label": bundle["atrvt_s2_label"],
        "atrvt_spec": bundle["atrvt_spec"],
        "atrvt_label": bundle["atrvt_label"],
        "atr_scale": bundle["atr_scale"].astype(float),
        "atr_scale_s1": bundle["atr_scale_s1"].astype(float),
        "atr_scale_s2": bundle["atr_scale_s2"].astype(float),
        ...
        "s1_gate_contract": bundle["s1_gate_contract"],
        "s1_gate_label": bundle["s1_gate_label"],
        "s1_gate_trace": bundle["s1_gate_trace"].copy(),
        "s1_gate_enabled": bool(bundle["s1_gate_contract"]["s1_gate_enabled"]),
        }
...
def summarize_table(default_rows: Dict, stress_rows: Dict | None, harsh_rows: Dict | None) -> pd.DataFrame:
    ...
    for spec in WEIGHT_CANDIDATES:
        ...
        rows.append({
            ...
            "atrvt_label": d["atrvt_label"],
            "atrvt_s1_label": d["atrvt_s1_label"],
            "atrvt_s2_label": d["atrvt_s2_label"],
            "s1_gate_label": d["s1_gate_label"],
...
def write_launch_audit(adopted_default: Dict, adopted_stress: Dict, adopted_harsh: Dict) -> None:
    lines = [
        ...
        f"  - `S1 + S2 ATR vol-targeting` enabled via `{adopted_default['atrvt_label']}`",
        f"    - `S1={adopted_default['atrvt_s1_label']}`",
        f"    - `S2={adopted_default['atrvt_s2_label']}`",
        f"  - `S1 gate contract = {adopted_default['s1_gate_label']}`",
        "  - sell-side `EMA250`",
...
def write_weight_audit(table: pd.DataFrame, best: pd.Series) -> None:
    ...
    lines = [
        ...
        f"  - `S1 + S2 ATR vol-targeting` enabled via `{table.loc[table['is_current_default'], 'atrvt_label'].iloc[0]}`",
        f"  - `S1 gate contract = {baseline['s1_gate_label']}`",
        "  - sell-side `EMA250`",
        "  - stable normal re-entry `close3`",
        "  - high-churn normal re-entry `strict EMA50 + breakout_4`",
        "  - `HV percentile >= 85` forces the high-churn gate earlier",
