...
def build_signal_snapshot() -> Dict:
    ...
    latest = {
        ...
        "atr_scale_s1": float(sim["atr_scale_s1"].reindex(idx).ffill().iloc[-1]),
        "atr_scale_s2": float(sim["atr_scale_s2"].reindex(idx).ffill().iloc[-1]),
        ...
        "s1_gate_enabled": bool(s1_gate_trace["s1_gate_enabled"].reindex(idx).ffill().iloc[-1]),
        "s1_gate_family": str(s1_gate_trace["s1_gate_family"].reindex(idx).ffill().iloc[-1]),
        "s1_gate_label": str(s1_gate_trace["s1_gate_label"].reindex(idx).ffill().iloc[-1]),
        "s1_gate_candidate": bool(s1_gate_trace["s1_candidate"].reindex(idx).fillna(False).iloc[-1]),
        "s1_gate_pass": bool(s1_gate_trace["s1_gate_pass"].reindex(idx).fillna(False).iloc[-1]),
        "s1_gate_reason": str(
            s1_gate_trace["s1_gate_reason"].reindex(idx).fillna(
                "disabled" if not bool(bundle["s1_gate_contract"]["s1_gate_enabled"]) else "not_applicable"
            ).iloc[-1]
        ),
        "s1_vp_hvn_escape_atr": (
            float(s1_gate_trace["s1_vp_hvn_escape_atr"].reindex(idx).iloc[-1])
            if pd.notna(s1_gate_trace["s1_vp_hvn_escape_atr"].reindex(idx).iloc[-1])
            else None
        ),
        "s1_vp_volume_ratio20": (
            float(s1_gate_trace["s1_vp_volume_ratio20"].reindex(idx).iloc[-1])
            if pd.notna(s1_gate_trace["s1_vp_volume_ratio20"].reindex(idx).iloc[-1])
            else None
        ),
    ...
    recent_4h["atr_scale_s1"] = sim["atr_scale_s1"].reindex(recent_4h.index).ffill().fillna(1.0).astype(float)
    recent_4h["atr_scale_s2"] = sim["atr_scale_s2"].reindex(recent_4h.index).ffill().fillna(1.0).astype(float)
    ...
    recent_4h["s1_gate_candidate"] = s1_gate_trace["s1_candidate"].reindex(recent_4h.index).fillna(False)
    recent_4h["s1_gate_pass"] = s1_gate_trace["s1_gate_pass"].reindex(recent_4h.index).fillna(False)
    recent_4h["s1_gate_reason"] = s1_gate_trace["s1_gate_reason"].reindex(recent_4h.index).fillna(
        "disabled" if not bool(bundle["s1_gate_contract"]["s1_gate_enabled"]) else "not_applicable"
    )
    recent_4h["s1_vp_hvn_escape_atr"] = s1_gate_trace["s1_vp_hvn_escape_atr"].reindex(recent_4h.index)
    recent_4h["s1_vp_volume_ratio20"] = s1_gate_trace["s1_vp_volume_ratio20"].reindex(recent_4h.index)
    ...
    payload = {
        ...
        "recent_ohlc": recent_4h[["open", "high", "low", "close", "ema250", "ema50", "core_target", "core_gate", "core_instability_state", "core_instability_source", "atr_scale", "atr_scale_s1", "atr_scale_s2", "hv_pct_180d", "s1_weight", "s2_weight", "s1_gate_candidate", "s1_gate_pass", "s1_gate_reason", "s1_vp_hvn_escape_atr", "s1_vp_volume_ratio20"]],
