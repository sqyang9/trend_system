...
- Adopted sleeve scaling:
  ...
  - `S1 gate = S1_VP_LB60_EA050_VR120`
...
- As of `2026-04-04`, the current locked official mainline is the balanced asymmetric ATRVT package:
...
- Mainline freeze rule:
  - do not replace the current official mainline unless a challenger clears promotion audit, landing audit, implementation parity, and documentation sync
- The former `S1 volume-profile proxy gate` reserve line is now promoted into the locked baseline.
- Governance companions:
  - runtime verification: `MAINLINE_RUNTIME_VERIFICATION.md`
  - monitoring checklist: `MAINLINE_MONITORING_CHECKLIST.md`
  - risk notes: `MAINLINE_RISK_NOTES.md`
- When there is any ambiguity between old hybrid docs, earlier combo docs, balanced ATRVT docs, and the current `S1 volume-profile proxy gate` docs, the current promoted `S1_VP_LB60_EA050_VR120` package wins.
...
- Use the adopted asymmetric ATRVT contract `S1 90d / S2 60d` as the canonical sleeve-scaling reference unless a newer promotion explicitly supersedes it.
